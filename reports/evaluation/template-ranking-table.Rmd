...
ranges <- c(0, seq(10, 90, by = 10), 95, 98)

df <- df %>%
  # Check all quantiles per target/location
  group_by(location, target_variable, target_end_date, model, boundary) %>%
  mutate(all_ranges_present = (length(setdiff(ranges, range)) == 0)) %>%
  group_by(location, target_variable, target_end_date, model) %>%
  mutate(all_ranges_present = all(all_ranges_present)) %>%
  ungroup() %>%
  filter(all_ranges_present == TRUE) %>%
  select(-all_ranges_present)
