...
The following table shows scores averaged across all locations, for all models that make forecasts for more than half of countries in the Hub.
...
# Performance by country {.tabset .tabset-fade .tabset-dropdown}

```{r by_country, include = FALSE, eval = include_countries, results = 'asis'}
out <- NULL
for (loc in unique(data$location_name)) {
  out = c(out, paste("\n\n##", loc, "\n\n"))
  out = c(out, knit_child(here::here("reports", "evaluation", "evaluation-by-country.Rmd"),
                          quiet = TRUE))
}
```

`r paste(if (include_countries) knit(text = out, quiet = TRUE), collapse = '\n\n')`
