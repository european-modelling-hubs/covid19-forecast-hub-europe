...
date: "`r lubridate::floor_date(lubridate::today(), 'week', week_start = 1)`"
---
...
report_date <- lubridate::floor_date(today(), 'week', week_start = 1)
```
...
forecasts[, horizon := paste0(horizon, " week", if_else(horizon == 1, "", "s"))]
forecasts[, ago :=
              as.integer(floor_date(today(), "week", week_start = 1) -
                         forecast_date) / 7]
forecasts[, ago := paste0(ago, " week", if_else(ago == 1, " ago", "s ago"))]
forecasts[, ago := if_else(ago == "0 weeks ago", "This week", ago)]
...
data <- scoringutils::merge_pred_and_obs(forecasts, truth, 
                                         join = "full")
data[target_variable == "inc case", target_variable := "Cases"]
data[target_variable == "inc death", target_variable := "Deaths"]
...
agos <- sort(unique(data$ago[!is.na(data$ago)]))
if ("This week" %in% agos) {
  agos <- c("This week", setdiff(agos, "This week"))
}
target_variables <- unique(data$target_variable)
