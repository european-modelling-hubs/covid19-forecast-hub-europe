...
class Predictor:
    ...
    def predict_variational_model(self, country, num_preds=1, model=None):
        ...
        for col in self.config.target_cols:
            ...
            plot_col_index = self.window.column_indices[col]
            if self.window.label_columns:
                label_col_index = self.window.label_columns_indices.get(col, None)
            else:
                label_col_index = plot_col_index
            samples = predictions[:, :, label_col_index]
            posterior_quantile = np.percentile(samples, self.config.quantiles, axis=-1)
            self.window.print_bayesian_prediction(observed=y_test_desc, predictions=posterior_quantile,
                                                  plot_col=label_col_index)
            self.window.plot_bayesian_prediction(prior_observed=x_test_desc, observed=y_test_desc,
                                                 predictions=posterior_quantile, plot_col=label_col_index,
                                                 plot_col_name=col)
            self.create_forecasts_file(country=country, num_preds=1, values=posterior_quantile,
                                       plot_col=label_col_index, quantiles=self.config.quantiles)

        return

    def create_forecasts_file(self, country, values, num_preds=1, quantiles=[2.5, 25, 50, 75, 97.5], plot_col=0):
        ...
        with open(forecast_filename, append_write) as file:
            ...
            for i in range(len(values)):
                ...
                if i == 0:
                    file.write("point,NA,")
                    if values[self.config.median_quantile_index, plot_col] < 0:
                        file.write(str(-values[self.config.median_quantile_index, plot_col]) + "\n")
                    else:
                        file.write(str(values[self.config.median_quantile_index, plot_col]) + "\n")
                    if len(values) > 1:
                        file.write("forecast,")
                        ...
                        if values[i] < 0:
                            file.write(str(-values[i, plot_col]) + "\n")
                        else:
                            file.write(str(values[i, plot_col]) + "\n")
                else:
                    file.write("quantile,")
