...
coverage <- scoringutils::score(
    data |>
      filter(!is.na(true_value)),
    metrics = "coverage"
  ) %>%
  filter(!is.na(range), range %in% ranges) %>%
  summarise_scores(by = c("model", "range", "target_variable", "horizon")) %>%
...
if (any(scores$model == params$model)) {
  ...
  targets <- coverage %>%
    ...
    unique()
  coverage <- coverage %>%
    dplyr::filter(target_variable %in% targets) %>%
    select(model, range, variable, horizon, coverage) %>%
    distinct() %>%
    mutate(model =
             factor(model, levels = unique(c(ensemble_model, params$model))),
  ...
  cat("No coverage figures shown as no 50%/95% predictive intervals are available.\n")
}
```
  ...
  if (length(setdiff(quantiles, core_quantiles$quantile)) == 0) {
    pit_data <- data %>%
      ...
      mutate(horizon = paste0(horizon, " week", if_else(horizon == 1, "", "s")),
             model = factor(
               model,
               levels = unique(c(ensemble_model, params$model)))
             ) %>%
      filter(!is.na(model))  %>%
      mutate(variable =
               forcats::fct_recode(target_variable, !!!target_variables))  %>%
      pit(by = c("model", "variable", "horizon"))

    p <- ggplot(pit, aes(x = quantile, y = pit_value, fill = model)) +
