...
for (fcast_date in as.character(forecast_dates)) {
  ...
  for (target in names(target_variables)) {
    ...
    filter_truth <- list(paste0("target_end_date > '", as.Date(fcast_date) - 7 * 10, "'"),
                         paste0("target_end_date <= '", as.Date(fcast_date) + 7 * 4, "'"))
     
    filter_forecasts <- list(paste0("forecast_date == '", as.Date(fcast_date), "'"))
    if (!is.null(params$model)) {
      filter_forecasts <- c(filter_forecasts,
                            list(paste0("model == '", params$model, "'")))
    }
    
    plot <- plot_data <- scaled_data |>
      scoringutils::make_NA(
        target_variable != target_variables[[target]], what = "both"
      ) |>
      scoringutils::make_NA(
