...
# get location codes
codes = pd.read_csv('././template/locations_eu.csv')['iso3c']
...
def covid19_row_validator(column_index_dict, row, codes):
    ...
    if location not in codes:
        error_messages.append(f"invalid location: {location!r}. row={row}")
    ...
    if [target_end_date.weekday()] != 6:  # Sunday
        error_messages.append(f"target_end_date was not a Sunday: {target_end_date}. row={row}")
    ...
    if target_end_date != exp_target_end_date:
        error_messages.append(f"target_end_date was not the expected Sunday. forecast_date = {forecast_date}, "
