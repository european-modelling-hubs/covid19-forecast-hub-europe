...
ggplot(cov, aes(x = forecast_date, y = cov_50)) +
  ...
  geom_hline(data = overall, aes(yintercept = cov_50), linetype = "dotted") +
  ylab("50% coverage") +
  scale_x_date(
    "Forecast date",
...
ggplot(cov, aes(x = forecast_date, y = cov_95)) +
  ...
  geom_hline(data = overall, aes(yintercept = cov_95), linetype = "dotted") +
  ylab("95% coverage") +
  scale_x_date(
    "Forecast date",
    labels = scales::label_date_short()
...
```{r wis, echo = FALSE}
ggplot(ensemble_evaluation, aes(x = evaluation_date, y = rel_wis)) +
  geom_line() +
  facet_grid(target_variable ~ horizon) +
  theme_bw() +
  scale_x_date(
...
```{r ae, echo = FALSE}
ggplot(ensemble_evaluation, aes(x = evaluation_date, y = rel_ae)) +
  geom_line() +
  facet_grid(target_variable ~ horizon) +
  theme_bw() +
  scale_x_date(
